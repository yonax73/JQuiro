define(["require", "exports", '.././base_component/JQElement', '.././icon/jqicon'], function(require, exports, jqE, jqI) {
    ...
    var jqBranch = (function () {
        function jqBranch(_jqLabel) {
            ...
            this.branchChildrenList = new Array();
        ...
        jqBranch.prototype.addBranch = function (_branch) {
            ...
            var _pos = this.branchList.push(_branch);
            this.branchChildrenList.push(_pos - 1);
        ...
        jqBranch.prototype.expandBranchs = function () {
            if (this.isCollapse()) {
                ...
                if (this.isExpandAll()) {
                    ...
                    var _length = this.getChildrenLength();
                    var _tmpListChildren = this.getChildrenList();

                    while (i < _length) {
                        var _branch = _tmpListChildren[i];
                        _branch.setOpen(false);
                        _branch.getBranch().removeClass(jqE.jqStyle.JQ_HIDE);
                        _branch.getBranch().addClass(jqE.jqStyle.JQ_SHOW);
                        i++;
        ...
        jqBranch.prototype.getChildrenList = function () {
            var _lengthChildren = this.getChildrenLength();

            var _tmpBranchList = new Array();

            if (_lengthChildren > 0) {
                var _length = this.branchList.length;
                var _i = 0;
                var _j = 0;
                var _k = 0;
                ...
                for (_i = 0; _i < _length; _i++) {
                    if (_k < _lengthChildren) {
                        for (_j = _k; _j < _lengthChildren; _j++) {
                            if (_i === this.branchChildrenList[_j]) {
                                _tmpBranchList.push(this.branchList[_i]);
                                _k++;
                            }
                        }
                    } else {
                ...
                _i = _j = _k = _lengthChildren = _length = null;
            }

            return _tmpBranchList;
        };

        jqBranch.prototype.getChildrenLength = function () {
            return this.branchChildrenList.length;
        };
        ...
        jqBranch.prototype.isExpandAll = function () {
            return this.expandAll;
        };

        jqBranch.prototype.setExpandAll = function (_expand) {
            this.expandAll = _expand;
        };
